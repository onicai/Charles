...
output=$(dfx canister call bioniq_mock ready --network $NETWORK_TYPE)

if [ "$output" != "(variant { Ok = record { status_code = 200 : nat16 } })" ]; then
    echo "bioniq_mock is not ready. Exiting."
    exit 1
else
    echo "bioniq_mock is ready for inference."
fi

# echo " "
# echo "--------------------------------------------------"
# echo "Setting NUM_LLMS_ROUND_ROBIN to $NUM_LLMS_ROUND_ROBIN"
# output=$(dfx canister call bioniq_mock setRoundRobinLLMs "($NUM_LLMS_ROUND_ROBIN)" --network $NETWORK_TYPE)
...
do
    ...
    do
        (  # Start a subshell to run the commands in parallel for each llm
            ...
            # IDENTITY="demo$i"
            # command="dfx canister call --identity $IDENTITY bioniq_mock Inference \"(record {prompt=\\\"\\\"; steps=60; temperature=0.0; topp=0.9; rng_seed=0})\" --network $NETWORK_TYPE"
            STORY_ID="$i-$i"
            STORY_PROMPT=$(get_random_prompt)
            # command="dfx canister call bioniq_mock NFTUpdate \"(record {token_id=\\\"$TOKEN_ID\\\"})\" --network $NETWORK_TYPE"
            command="dfx canister call bioniq_mock StoryUpdate \"(record {storyID=\\\"$STORY_ID\\\"; storyPrompt=\\\"$STORY_PROMPT\\\"})\" --network $NETWORK_TYPE"
